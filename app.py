...
def fetch_patient_by_id(db, patient_id):
    ...
    select_patient_query = "SELECT * FROM patient WHERE patient_id = %s"
...
def delete_patient_record(db, delete_option, delete_value):
    ...
    if delete_option == "ID":
        delete_patient_query = "DELETE FROM patient WHERE patient_id = %s"  # Update this line if the ID column is named differently
...
def insert_appointment_record(db, patient_id, appointment_date, appointment_time, doctor_name, doctor_id,notes):
...
def update_patient_record(db):
    ...
    if st.button("Search :magic_wand:"):
        ...
        if patient:
            ...
            df = pd.DataFrame([patient], columns=[ 'ID', 'Name', 'Birth Date', 'Contact Number', 'Email', 'Address 1', 
                'Address 2', 'City', 'Postal Code', 'Country', 'Work Phone', 
                'Home Phone', 'Emergency Contact Person', 'Emergency Contact Phone', 
                'Insurance ID', 'Insurance Owner', 'Date Added', 'Patient Owner Relation'])
            st.dataframe(df)
...
def edit_patient(db):
    ...
    new_birth_date = st.date_input("Enter new birth date",key = "birth_date",value=None,min_value=datetime.date(year=1919,month=12,day=31))
...
def main():
    ...
    if options== "Home":
       ...
       birth_date = st.date_input("Enter birth date of patient",key = "birth_date",value=None,min_value=datetime.date(year=1919,month=12,day=31))
       ...
       if st.button("add patient record"):
        ...
        if patient:
         ...
         if st.button("Delete"):
          ...
          doctor_id = st.number_input("Enter doctor's ID:", key="appointment_doctor_id")
          if st.button("Add Appointment"):
               insert_appointment_record(db, patient_id, appointment_date, appointment_time, doctor_name, doctor_id, notes)
